            ...
            @foreach($reviews as $review)
                <div class="flex items-center justify-between p-5 bg-white rounded-xl shadow-lg hover:shadow-xl transition duration-300 ease-in-out border border-gray-100">
                    ...
                    <div class="flex space-x-3 ml-6 flex-shrink-0">
                        ...
                        <a href="{{ route('reviews.edit', $review->id) }}" class="px-4 py-2 text-sm font-medium text-white bg-primary rounded-lg hover:bg-primary-hover transition duration-150 shadow-md">
                            Editar
                        </a>

                        <form action="{{ route('reviews.destroy', $review->id) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja DELETAR a avaliação {{ $review->user_id }}?')" class="inline">
                            @csrf
                            @method('DELETE')
