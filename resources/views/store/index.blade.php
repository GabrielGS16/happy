...
<head>
    ...
    <title>Lista de loja (Tailwind)</title>
...
<body class="bg-bg-light min-h-screen flex flex-col items-center p-6">
    ...
    <div class="w-full max-w-4xl">
            ...
            @foreach($stores as $store)
                <div class="flex items-center justify-between p-5 bg-white rounded-xl shadow-lg hover:shadow-xl transition duration-300 ease-in-out border border-gray-100">
                    ...
                    <div class="flex-grow min-w-0">
                        <p class="text-xl font-semibold text-gray-800 truncate" title="{{ $store->name }}">
                            {{ $store->name }}
                        </p>
                        <p class="text-sm text-gray-500 mt-1 line-clamp-2" title="{{ $store->description }}">
                            {{ $store->description }}
                        </p>
                    </div>
                    ...
                    <div class="flex space-x-3 ml-6 flex-shrink-0">
                        ...
                        <a href="{{ route('stores.edit', $store->id) }}" class="px-4 py-2 text-sm font-medium text-white bg-primary rounded-lg hover:bg-primary-hover transition duration-150 shadow-md">
                            Editar
                        </a>

                        <form action="{{ route('stores.destroy', $store->id) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja DELETAR a loja {{ $store->name }}?')" class="inline">
                            @csrf
                            @method('DELETE')
        ...
        <div class="mt-10 pt-6 border-t border-gray-200 flex justify-center space-x-8">
            <a href="{{ route('stores.create') }}" class="flex items-center text-primary font-semibold hover:text-primary-hover transition duration-150">
